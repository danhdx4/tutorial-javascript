...
test("Should be create a articel successfully", async ({ page, request }) => {
    // navigate to the creating article page. Đi đến trang tạo bài viết
    await page.getByRole("link", { name: " New Article " }).click();

    // fill all data. Điền thông tin bài viết
    ...
    // click to 'public article' btn. Bấm nút Đăng bài
    await page.getByRole("button", { name: " Publish Article " }).click();
    const createAritcleResponse = await page.waitForResponse(
        "https://conduit-api.bondaracademy.com/api/articles/",
    );
    const createAritcleResponseBody = await createAritcleResponse.json();// Chờ API trả về thông tin bài viết. slug – giống như ID của bài viết.
    const slug = createAritcleResponseBody.article.slug;

    // Back to Home page and verify the created article is existing . Quay lại trang chủ kiểm tra bài viết có xuất hiện hay không.
    await page.goto("https://conduit.bondaracademy.com/");
    await expect(
    ...
    // Tear down: Clean data after testing | Delete the article . Xóa bài viết bằng API
    // Get access token by the loginn API . Lấy tocken đăng nhập
    ...
    // Delete the article by the delete API . Gửi yêu cầu DELETE đến API. Nếu xóa thành công, API trả về 204 
