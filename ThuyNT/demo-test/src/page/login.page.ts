...
export class LoginPage extends basePage {
    ...
    emailErrormsg = this.page.locator('.form-control-group').filter({ hasText: 'Email address:' }).locator('.caption.status-danger')
    passwordErrormsg = this.page.locator('.form-control-group').filter({ hasText: 'Password:' }).locator('.caption.status-danger')
    checkBox =  this.page.getByRole("checkbox", { name: "Remember Me" });
    ...
    async verifyErrMsg(field: "email" | "password", msg: string) {
        ...
        const errMsg = this.page.locator('.form-control-group').filter({ hasText: `${textField}` }).locator('.caption.status-danger')
        await expect(errMsg).toHaveText(msg)
    }

    async verifySubmitBtn(isStatus: "enable" | "disable") {
        const colorValue = isStatus === "enable" ? "rgb(51, 102, 255)" : "rgba(143, 155, 179, 0.24)"
