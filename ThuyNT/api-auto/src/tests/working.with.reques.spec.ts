...
test.describe('Hanldle with article', () => {
    test("Should be create a articel successfully", async ({ page, request, login, loginByAPI }) => {
    ...
    test('Should delete the article successfully', async ({ page, request, login, loginByAPI }) => {
        await page.goto("https://conduit.bondaracademy.com/");
        // Set up
        ...
        const createArticleResponse = await request.post(
            `https://conduit-api.bondaracademy.com/api/articles/`,
            {
                ...
                data:
                {
                    "article":
                    {
                        "title": `Automation Testing for API ${Date.now()}`,
